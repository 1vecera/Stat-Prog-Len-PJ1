...
Player_latest <- data.frame(Player_latest, fit$cluster)

# vary parameters for most readable graph
library(cluster) 
clusplot(Player_latest, fit$cluster, color=TRUE, shade=TRUE, 
         labels=2, lines=0)

library(fpc)
plotcluster(Player_latest, fit$cluster)

# how many of each group?
summary(as.factor(Player_latest$fit.cluster))
